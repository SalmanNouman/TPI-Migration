...
var debug = true;   // value is true when building a 'Development Build' in Unity
#else
...
function doGetValue(identifier, objectname, callbackname, key) {
    message("SCORM JS : Getting value with {" + identifier + ", " + objectname + ", " + callbackname + ", " + key + "}");
    ...
    if (api == null) {
        ...
        if (error.code != _NoError.code) {
            ...
            window.unityInstance.SendMessage(objectname, callbackname, result + "" + "|" + error.code + "|" + error.string + "|" + key);
        } else {
            window.unityInstance.SendMessage(objectname, callbackname, result + "" + "|" + "|" + "|" + key);
        }
    }
}
...
function doSetValue(identifier, value, objectname, callbackname, randomnumber) {
    message("SCORM JS : Setting value with {" + identifier + ", " + value + ", " + objectname + ", " + callbackname + ", " + randomnumber + "}");
    // JP TODO - temp hack to get rid of the value for strings and associated language data model elements
    var dotBindingName = identifier.replace(".Value", "");
    var api = getAPIHandle();
    var result = "false";
    ...
    if (api == null) {
        ...
        if (result.toString() != "true") {
            ...
            window.unityInstance.SendMessage(objectname, callbackname, result.toString() + "|" + err.code + "|" + err.string + "|" + randomnumber);
        } else {
            window.unityInstance.SendMessage(objectname, callbackname, result.toString() + "|" + "|" + "|" + randomnumber);
        }
    }
}
...
function doIsScorm2004(objectname, callbackname, randomnumber) {
    message("SCORM JS : Checking scorm version with values " + objectname + ", " + callbackname + ", " + randomnumber);
    var api = getAPIHandle();
    var result = "false";
    if (api == null) {
    ...
    window.unityInstance.SendMessage(objectname, callbackname, versionIsSCORM2004 + "|" + "|" + "|" + randomnumber);
}
...
var scorm = (function () {
    return {
        foo: function (str) {
            alert(str);
            window.unityInstance.SendMessage("GameObject", "fromweb", "message from web");
        }
    };
